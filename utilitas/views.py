...
class BaseView(APIView, CustomPagination):
        ...
        for i in body_params:
            if i["operator"] == "in":
                ...
                params_dict[i["field_name"] + "__" + i["operator"]] = (
                    True if i["value"] == "true" else False
                )
            else:
                params_dict[i["field_name"] + "__" + i["operator"]] = i["value"]
    ...
    def get_queryset(
        ...
        if not is_csv:
            ...
            # if self.serializer.role_based_fields and roles:
            #     authorized_fields = []
            #     for i in roles:
            #         authorized_fields.extend(self.serializer.role_based_fields.get(i, []))
            #     if len(fields) == 0:
            #         fields = self.model.get_fields(self.model)
            #     fields = set(fields).difference(set(authorized_fields))
            # print(fields)
    ...
    def get_serializer(self, *args, **kwargs):
        ...
        kwargs.setdefault(
            "context", {**self.get_serializer_context(), **original_context}
        )
